...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        self.reg = reg
        self.num_layers = 1 + len(hidden_dims) #num_layers equals hidden_layer sum output layer(except input layer) 
        self.dtype = dtype
        self.params = {}
        ...
        ############################################################################
        #self.num_layers=len(hidden_dims)+1 bn需要对输入？一般都是接在卷积层后面的吧，话说最后一层要不要加bn
#         for layer in range(self.num_layers):
#             self.params['W%d'%(layer+1)] = np.random.normal(0.0, weight_scale, (input_dim, hidden_dims[layer]))
...
#             input_dim = hidden_dims[layer]
        ...
        for i, hd in enumerate(hidden_dims):
            self.params['W%d'%(i+1)] = weight_scale * np.random.randn(layer_input_dim, hd) #(n,layer_dim)
        ...
        # With batch nomalization we need to keep track of running means and
        ...
        if self.normalization=='layernorm':
            self.bn_params = [{} for i in range(self.num_layers - 1)]# why is it different between bn and ln?
    ...
    def loss(self, X, y=None):
        ...
        for layer in range(self.num_layers-1):
            if self.normalization == 'batchnorm':
                layer_input, ar_cache[layer] = affine_bn_relu_forward(layer_input, self.params['W%d'%(layer+1)], self.params['b%d'%(layer+1)], 
            ...
            if self.use_dropout:
                layer_input, dp_cache[layer] = dropout_forward(layer_input, self.dropout_param)
        
        # the last layer do not use batch normalization
        ...
        #idx from 0 to num_layers-1, lay from num_layers-2 to -1, lay+1
