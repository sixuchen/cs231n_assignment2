...
#    dw = np.dot(x.T,dout).reshape(w.shape)
    dw=np.dot(np.reshape(x, (x.shape[0], -1)).T,dout) #先rshape再乘和先乘再reshape不一样？
    #db = dout
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        # running_mean以及running_var不需要存到bnparam中吗？
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
